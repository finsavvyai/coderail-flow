...
describe('FlowRecorder utils', () => {
  it('builds proxy URLs with normalized API bases', () => {
    expect(
      getProxyUrl('https://app.example.com/login?redirect=%2Fhome', 'https://api.example.com')
    ).toBe('https://api.example.com/proxy/aHR0cHM6Ly9hcHAuZXhhbXBsZS5jb20/login?redirect=%2Fhome');
  });
