...
describe('getRecorderRuntimeConfig', () => {
  ...
  it('prefers proxy-backed recording in production', () => {
    const config = getRecorderRuntimeConfig(
      makeEnv({
        PROD: true,
...
describe('recorder runtime helpers', () => {
  it('normalizes external bases and target URLs', () => {
    ...
    expect(normalizeExternalBase('https://api.example.com/base/')).toBe(
      'https://api.example.com/base'
    );
    expect(normalizeRecordableUrl('https://app.example.com/login')).toBe(
