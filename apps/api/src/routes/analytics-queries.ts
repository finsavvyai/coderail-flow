/** Analytics query builders for step, element, and run analytics. */

import type { Env } from '../env';
import { q } from '../db';
import { toNumber, normalizeTimeRangeToDays } from './helpers';

export async function buildStepAnalyticsPayload(env: Env, timeRange?: string, projectId?: string) {
...
export async function buildElementReliabilityPayload(env: Env, projectId?: string) {
  let whereClause = '';
  const params: any[] = [];
  if (projectId) {
...
export async function buildAnalyticsPayload(env: Env, timeRange?: string, projectId?: string) {
  const days = normalizeTimeRangeToDays(timeRange);
  const since = new Date(Date.now() - days * 24 * 60 * 60 * 1000).toISOString();
  let whereClause = 'WHERE r.created_at >= ?';
  const params: any[] = [since];
